...
export function WatchPlayer({
  ...
  useEffect(() => {
    ...
    v.currentTime = 0;
    player.resetRate();
    autoplay(v, clip.video, clip.poster);
    setMuted(true);
  }, [clip?.video, clip?.poster, clipId]);
  ...
  function pickChoice(nextId: string) {
  ...
  return (
    ...
    >
      <video
        ...
        src={clip?.video || undefined}
        playsInline
        muted
        autoPlay
        preload="auto"
        poster={clip?.poster || series.cover}
      ...
      {!ended ? (
        <div className="pointer-events-none absolute inset-x-0 bottom-28 z-20 px-5">
          <div className="text-[11px] text-white/55">{series.tag} · 互动学习</div>
          <div className="mt-1 text-xl font-semibold">{clip?.title}</div>
          <p className="mt-1 line-clamp-3 text-sm text-white/75">{clip?.script}</p>
        </div>
      ) : (
        <div data-player-ui className="absolute inset-x-0 bottom-24 z-30 px-4">
          ...
          <div className="space-y-2">
            {(clip?.choices ?? []).map((c, i) => (
              <button
                key={c.id}
                type="button"
                onClick={() => pickChoice(c.nextClipId)}
      ...
      <SeekBar
        ...
        className={ended ? "bottom-[13.5rem]" : "bottom-[4.6rem]"}
      />
